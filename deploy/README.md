...
Ubuntu / Debian. Бот нетребователен: около 100 МБ памяти, диска — меньше гигабайта.

## Быстрый способ

```bash
git clone -b claude/discord-auth-bot-JPFKq https://github.com/Jeredpoi/DsAuth.git /tmp/dsauth
sudo bash /tmp/dsauth/deploy/setup.sh
```

Скрипт спросит токен и твой Discord ID, всё остальное сделает сам: поставит
пакеты, создаст пользователя, окружение, автозапуск и ежедневные бэкапы.

Он идемпотентный — повторный запуск обновляет код и зависимости, не трогая
`.env` и базу. То есть это же и команда обновления:

```bash
sudo bash /opt/dsauth/deploy/setup.sh
```

Ниже — те же шаги вручную, если хочется контролировать каждый.
...
```bash
curl localhost:1324/health
```

Отдаёт имя бота, число серверов и пинг. Если хочешь подключить внешний
мониторинг (UptimeRobot и подобные) — открой порт 1324 наружу. Для работы
